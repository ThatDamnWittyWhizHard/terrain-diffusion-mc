...
public final class TerrainRiverBuilder {
    ...
    public static TerrainRiverTile build(TerrainFlowTile flow) {
        ...
        cleanLakeComponents(width, height, lake, ocean, discharge, terrainHeight, filledHeight);
        for (int idx = 0; idx < len; idx++) {
            if (lake[idx]) {
                river[idx] = false;
                waterLevelY[idx] = filledHeight[idx];
            } else {
                waterLevelY[idx] = terrainHeight[idx];
            }
            riverPotential[idx] = Math.max(riverPotential[idx], lake[idx] ? 0.88F : 0.0F);
        }
    ...
    private static void cleanLakeComponents(
            int width,
            int height,
            boolean[] lake,
            boolean[] ocean,
        ...
        boolean[] visited = new boolean[lake.length];
        int[] queue = new int[lake.length];
        List<Integer> cells = new ArrayList<>();

        for (int start = 0; start < lake.length; start++) {
            if (!lake[start] || visited[start]) {
                continue;
            }
            ...
            while (head < tail) {
                ...
                maxDepth = Math.max(maxDepth, filledHeight[cell] - terrainHeight[cell]);
                maxDischarge = Math.max(maxDischarge, discharge[cell]);
            ...
            int componentHeight = maxZ - minZ + 1;
            boolean tooSmall = cells.size() < TerrainRiverConfig.MIN_LAKE_SURFACE_CELLS
                    || componentWidth < TerrainRiverConfig.MIN_LAKE_COMPONENT_WIDTH_BLOCKS
                    || componentHeight < TerrainRiverConfig.MIN_LAKE_COMPONENT_WIDTH_BLOCKS;
            boolean tooWeak = maxDischarge < TerrainRiverConfig.MIN_POND_DISCHARGE
                    && maxDepth < TerrainRiverConfig.MIN_LAKE_DEPTH_BLOCKS * 1.8F;
            ...
            // Oceans are generated by biome/profile and must never be downgraded into lake cells.
            for (int cell : cells) {
                if (ocean[cell]) {
                    lake[cell] = false;
                }
